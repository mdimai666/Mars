...
- Планы (`*Plan.md`) живут, пока работа идёт; по закрытии инициативы схлопываются в гайд
  подсистемы (`*Guide.md`), а планы удаляются — направление и чек-лист в
  `ai/PlanLifecycleGuide.md`.
- Гайды пишутся для агента, не для человека: буллеты с путями файлов, разделы «Грабли»,
