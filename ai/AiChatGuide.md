...
модальное окно внизу по центру (перетаскивается за шапку; позиция живёт только в памяти —
переживает свернуть/развернуть, но не перезагрузку страницы): строка ввода `❯` + Enter, стриминг
ответа, строки вызовов инструментов, кнопки «стоп» / «новый чат» / переключение чатов.
