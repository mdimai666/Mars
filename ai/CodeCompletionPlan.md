...
## Решения и замер памяти (2026-09-25, конец сессии)

**Отложено (решения пользователя):**
- **Инкрементальные `WithChanges` — НЕ делаем.** Почему: скрипты нод — десятки строк, полный
  репарсинг Roslyn — единицы мс; замеренные ~0.3 с на повторный запрос — это completion/анализ +
  HTTP + сериализация, а не парсинг. Документ и так обновляется на месте, Roslyn кэширует
...
- **Idle-освобождение контекста — СДЕЛАНО (2026-09-25)**, см. секцию «Idle-освобождение контекста»
  ...
  `CodeCompletionWorkspaceManager` singleton (Dispose только на shutdown), MEF-`Lazy` без сброса,
  `ContextWorkspace` (AdhocWorkspace + ~300 MetadataReference + CompletionTypeIndex) живёт до
  смерти процесса → сценарий «отредактировал один раз, процесс живёт месяцами» держит ~1 ГБ
  private. Решение на будущее: idle-таймер — нет документов и запросов N часов → dispose
  ContextWorkspace (возвращает основную часть; загруженные Features-сборки и JIT не выгружаются
  из процесса, это десятки МБ), опционально сброс MEF-`Lazy`; следующая активация платит цену
  первого запроса (~секунды). Сейчас не горит: в prod флаг `CodeCompletion: false` →
  инфраструктура не создаётся вообще.
- **Типизация `msg` для подсказок — на будущее.** Дизайн готов (обсуждение 2026-09-25):
  уровень 1 — синтетический typed-глобалс (`NodeMsg msg` только для редактора, рантайм не трогаем)
  → `msg.` дополняется, hover/подсветка работают; грабля — flattened-доступ обёртки
  (`msg.SomePayloadProp`, ключи Context) покраснеет CS1061 → глушить их в DiagnosticsQueryService
  осознанно. Уровень 2 («что передали предыдущие ноды») — только после stage2-контрактов выходов
  нод (typed payload per-edge), эмпирика из debug-запусков отклонена как магия.

**Замер памяти (зонд-тест на контексте с полной TPA ≈ FunctionNode, x64, .NET 10):**

| Этап | Managed | Private bytes |
|---|---|---|
| Первый документ (MEF-хост + workspace + метаданные ~300 сборок + compilation + type index + первый completion) | **+108 МБ** | **+962 МБ** |
| Analyze (диагностика + semantic tokens) на нём же | +1,5 МБ | +20 МБ |
| Каждый следующий документ (свой проект+compilation) | <1 МБ (в пределах шума, ±) | <1 МБ (в пределах шума) |
| 20 правок текста одного документа | роста нет | роста нет |

Выводы: вся стоимость — **одноразовая инфраструктура на процесс** (лениво, при первом attach;
при выключенном флаге — ноль). Каждый дополнительный открытый редактор ≈ бесплатен; правки
