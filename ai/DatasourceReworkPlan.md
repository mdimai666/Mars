...
### Этап 8. Лимит строк при просмотре и «всего N» — сделано 2026-09-16

Решения пользователя: при открытии объекта показывать **50 строк** (спрашивал «или сколько принято»:
единого значения нет — phpMyAdmin ограничивает страницу 25 строками, а pgAdmin/DBeaver/ADS не ограничивают,
а догружают по мере прокрутки, потому что у них виртуализованный грид; наш грид рисует всё сразу, поэтому
ближе модель phpMyAdmin, а 20 тесновато для ячеек с json). Total — **только для «нашего» запроса** (вкладка
открыта кликом по объекту и её SQL не правили руками): у произвольного запроса непонятно, что считать;
меньше лимита — количество известно и так, ровно лимит — фоновый `COUNT(*)` с таймаутом 5 с;
«Показать больше» растит наш `LIMIT` × 5 (50 → 250 → 1250).

- [x] `BrowseSqlBuilder` (Abstractions, тестируется без Docker) — SQL просмотра с лимитом в самом запросе:
      `SELECT *` по первичному ключу + `LIMIT n` (psql/mysql) или `SELECT TOP n *` (mssql), и `Count(...)`
      по тому же объекту. Лимит в SQL даёт не только меньше трафика: с `ORDER BY` планировщик делает top-N
      вместо полной сортировки, а база не отдаёт лишние широкие строки.
- [x] `ViewDialect` из этапа 7 переименован в `SqlDialect` и вынесен в `SqlDialect.cs` вместе с
      `SqlDialectMapping` (`Dialect(driver)`, `Quote`, `Target`) — диалект потребовался вне DDL вьюх;
      `ViewDdlBuilder` переведён на него, второго набора правил квотирования не появилось.
- [x] `QueryTab`: `BrowseLimit` (50), `BrowseSql` (наш текст — он же признак «SQL не правили»), `Total`,
      `TotalNote`, `IsBrowse`, `CanLoadMore` (лимит просмотра **или** серверный `Truncated`).
      `OpenTableAsync` поднимает серверный предел до `BrowseLimit + 1` — иначе «Показать больше» за 500
      строк не показало бы ничего нового.
- [x] Грид: «строк: 20 из 1234» и «(считаем всего…)» на время подсчёта; «Показать больше» теперь по
      `CanLoadMore` (раньше только по `Truncated`, и в просмотре с лимитом кнопка исчезала).
- [x] Тесты: `SqlDialectMappingTests` (16 — драйвер→диалект, экранирование кавычек, квалификация схемы),
      `BrowseSqlBuilderTests` (9 — три диалекта, `TOP` против `LIMIT`, без ключей, составной ключ, лимит 0).
- Проверено: `dotnet build Mars.slnx` — 0 ошибок/0 предупреждений; юнит-наборы датасорса 58/58; движки —
      Postgres 11/11, MsSQL 7/7, MySQL 8/8. Поведение лимита, подсчёта и «Показать больше» в браузере
      проверяет пользователь (тестов на Razor-компоненты в репо нет).

### Этап 9. Схемы в дереве сворачиваются — сделано 2026-09-16

- [x] Заголовок схемы — кликабельная строка с шевроном; свёрнутым считается то, что перечислено
      в `_collapsedSchemas`, поэтому состояние принадлежит пользователю и не сбрасывается обновлением
      структуры (кэш, DDL, сохранение правок и т.п.).
- [x] По умолчанию раскрыта схема по умолчанию движка (`public`, у MsSQL — `dbo`), остальные свёрнуты;
      раскладка делается один раз на источник (`_schemaDefaultsFor`), иначе вручную свёрнутая схема
      разворачивалась бы после каждого обновления. Если схемы по умолчанию нет (в MySQL схема — это сама
      база), дерево остаётся раскрытым, как было; при единственной схеме заголовков нет вовсе
      (`hasMultipleSchemas`).
- [x] При активном фильтре дерево раскрыто целиком — иначе найденная таблица осталась бы спрятанной
      в свёрнутой схеме. Открытый объект тоже раскрывает свою схему (`OpenTableAsync`), иначе созданную
      вьюху было бы не видно.
