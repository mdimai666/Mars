...
сделаны целиком, включая фронт**; коммит `eb82a73d`. Проверено: `dotnet build Mars.slnx` — 0 ошибок/
0 предупреждений; `Mars.Datasource.Integration.Tests` 453/453; `Mars.WebApiClient.Integration.Tests`
...
7. ~~Документ `requests.http` из UI~~ — сделано в C7: вкладка-документ (весь `.http`), кнопка «.http»
   у дерева, переход к блоку по клику, «Выполнить блок» под курсором, «дубль»/«удалить блок», Ctrl+S.
   **Открыто:** нужна ли теперь форма параметров операции (она осталась под редактором и вписывает
   значения как переменные) — или её убрать, раз всё видно в тексте; и куда вставлять заготовку
   операции, которой в документе нет, — в конец (сейчас) или в позицию курсора.
