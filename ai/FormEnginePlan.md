...
- **D — реестры редакторов разделены по контрактам** (слияние было сделано и
  ...
  **обновлено 2026-09-11:** реестр — экземпляр-синглтон из DI (как локатор типов нод):
  регистрация через `Register` откуда угодно, словарь собирается в момент запроса.
...
`MetaValueEditModelLookup`, `MetaFieldTypePresets`, `MetaValueListHelper`) в слой
моделей Cms-админки, `GroupedSelectDropDown` в общие компоненты (у нод — своя
копия, так что потребитель один: `MetaFieldTypePicker`). Сама папка после
переноса — набор редакторов значений метаполей, т.е. то, что провайдер
регистрирует в общем движке; переименование папки — чистый churn по неймспейсам.
`MetaValueStore` (C2) и обобщение pull-протокола тяжёлых редакторов не
понадобились: зарегистрированные редакторы метаполей работают с EAV-строками из
каскадов, а протокол `IHeavyMetaValueEditors` теперь владеет и `MetaValuesForm`.
...
- **Ключи редакторов для реестра**: `GetDefaultEditor` для Relation/File/Image
  ...
  `editors.Register(key, component, multiple, title, types)`.
