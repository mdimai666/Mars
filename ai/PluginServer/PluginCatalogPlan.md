...
Статус: **Фазы 0–3 (сервис в моно-репо Mars.Cloud) завершены 2026-09-03**;
**Фазы 4–5 (сторона репо Mars) завершены 2026-09-03**; Фаза 6 — после инфраструктуры.
...
Тогда же Фазы 0–3 реализованы: витрина/синк с nuget.org, Keycloak-аутентификация, отзывы
и жалобы, подача с проверкой `packageType=MarsPlugin`, модерация, discovery-джоб; 73/73
тестов зелёные, синк проверен живым запросом к nuget.org.

Обновление 2026-09-03 (Фазы 4–5, репо Mars): ядро установщика из Фазы 4 было готово раньше
(реворк системы плагинов 2026-09-01/02: `PluginNugetInstaller`, `InstallFromNuget`, реестр
и жизненный цикл — см. `ai/PluginSystemGuide.md`), поэтому Фаза 4 сведена к интеграции
с каталогом. Отклонения от исходного текста фаз:

- **Настройки каталога — конфиг, не админ-опция**: секция `PluginCatalog` (`Url`, `Enabled`)
  в `appsettings.json` (`PluginCatalogOption`, `IOptions`); из админки не редактируется —
  решение пользователя. `PluginManagerSettingsOption` не тронута.
- **Проверка бана — без обхода каталога при установке**: публичное API каталога отдаёт
  только Approved/Recommended (забаненный неотличим от отсутствующего — 404), поэтому защита:
  забаненные не попадают в витрину (безопасно по построению) + локальная опция
  `BlockedPackageIds` для прямой установки по id. Статус-эндпоинт в каталог не добавлялся
  (это кросс-репо изменение; при желании — отдельная доработка Mars.Cloud).
- **Отзывы в витрине — только чтение** (рейтинг + текст без входа). Написание отзывов
  с входом через `<каталог>/auth/login` отложено в отдельную фазу (эскейп-хатч плана).
- Реализация: контракты `Mars.Plugin.Contracts.Catalog` (зеркало DTO каталога),
  `PluginCatalogClient` (Mars.Plugin; выключен/нет пакета → `null`, сбой связи или ошибка
  каталога → `UserActionException` с текстом — витрина показывает её с кнопкой «Повторить»),
  эндпоинты `GET api/Plugin/marketplace/status`, `marketplace`, `marketplace/{id}`,
  `marketplace/{id}/reviews` (роль Admin/Developer, фильтр совместимости — версия Марса
  как `minVersion`), методы в `IPluginServiceClient`,
  страница `/marketplace` (карточка хаба `/builder/` + пункт в тулбаре «Плагинов»;
  при выключенном каталоге — заглушка вместо витрины). Тесты: юниты клиента
  (`Mars.Plugin.Tests`) + интеграционка эндпоинтов при выключенном каталоге.
...
> Устарело: описано состояние до реворка системы плагинов (2026-09-01). Актуальное
> устройство — в `ai/PluginSystemGuide.md`; заглушка `InstallPlugin` закрыта, метаданные —
> в `mars-plugin.json` и атрибутах сборки.
...
**Выполнено 2026-09-03** (с отклонениями — см. обновление выше): установщик по NuGet-id
готов ещё с реворка плагинов (`PluginNugetInstaller` + `POST api/Plugin/InstallFromNuget`,
тесты раскладки в `Mars.Plugin.Tests`); из фазы сделана интеграция с каталогом:
`PluginCatalogOption` в `appsettings.json`, `PluginCatalogClient`, витринные эндпоинты.
Запрет установки — локальный `BlockedPackageIds` + «бан не показывается в витрине».
...
**Выполнено 2026-09-03** (с отклонениями — см. обновление выше): страница `/marketplace`
в `src/Mars.Admin/Pages/PluginViews/` (вход — карточка на хабе `/builder/` и пункт тулбара
страницы «Плагины»): поиск, фильтр recommended, сортировка, карточки с рейтингом/загрузками/
версией/тегами, диалог карточки с описанием и отзывами (чтение), установка/обновление
в один клик, сравнение установленных версий с `LatestVersion`. При `Enabled=false` —
сообщение вместо витрины. Написание отзывов отложено (флоу входа через каталог).
...
в Mars (установщик по NuGet-id + интеграция с каталогом + витрина `/marketplace`) —
выполнены 2026-09-03.
