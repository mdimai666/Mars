...
Фазы 4–5 `ai/PluginServer/PluginCatalogPlan.md` выполнены 2026-09-03: витрина маркетплейса
в админке (`/marketplace`, поиск/карточки/отзывы/установка в один клик) поверх каталога
(`PluginCatalogClient`, секция `PluginCatalog` в appsettings — Url и Enabled). Каталог
выключен — витрина показывает заглушку; недоступен/ошибается — страницу с ошибкой и
«Повторить»; установка по nuget-id от каталога не зависит.
Осталась Фаза 6 — деплой каталога и документация; написание отзывов в админке (вход через
каталог) — отдельная фаза.
