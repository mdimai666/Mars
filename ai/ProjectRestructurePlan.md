...
Следующие кандидаты из `Mars.WebApp` (обсуждено 2026-08-29): ~~CLI по владельцам~~ — ✅ сделано 2026-08-29 (см. ниже); ~~ядро в `Mars.Server`~~ — ✅ сделано 2026-08-29 (см. ниже); `RenderPageNodeImpl` → `WebApp.Nodes.Host`; далее — ревизия `AddMarsHostServices` (`AIToolService`, `DevAdminConnectionService`, `ModelInfoService`/`BlazorPagesService`), XActions (каталог упирается в типы страниц админки под `#if !NOADMIN`), хостинг админки (`StartupDevAdmin` + `StartupHostFiles` + `_AdminHost.cshtml`). Setup-визард — открыт (оставить в хосте или выделить в `Mars.Setup`).
...
- Верификация: сборка 0 ошибок; `GetSystemInfoTests` 18/18 (полный хост, сидинг через новый хендлер, `MarsSystemService`). **Известная проблема (пользователь чинит сам):** в `Mars.Cli.EndToEnd.Tests` падают 2 из 6 — `HelpFlag_NoServer_PrintedByNormalFlow` и `HelpFlag_ServerRunning_NotForwarded_PrintedByNormalFlow` (процесс `Mars.dll -h/--help` падает с кодом -532462766 без вывода); остальные 4 зелёные, включая полный запуск in-process (`node list --local`). Коммит — по команде пользователя.
