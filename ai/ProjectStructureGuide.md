...
`NodeTaskJob/`, `Nodes/`, `Hubs/`), `Abstractions`, `Expressions` (резолвер выражений), `Host`,
`Front.Abstractions` (фронтовый SDK редактора: интерфейсы, компоненты, wire-геометрия),
`Workspace` (Blazor-редактор), `FormEditor`.
