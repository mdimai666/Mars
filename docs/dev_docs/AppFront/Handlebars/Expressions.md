...
## Синтаксис выражений в ef-запросах

В условиях `Where`, `First`, `Last`, `Count` поддерживаются три формы записи:

```hbs
{{#context}}
a = ef.posts.Where(Title == "111")
b = ef.posts.Where(p => p.Title == "111")
c = ef.posts.Where(post.Title == "111")
{{/context}}
```

1. **Короткая форма** — `Title == "111"`: если выражение начинается с имени поля сущности, префикс не нужен. Работает и с вызовами методов: `Title.StartsWith("11")`, и с путями через точку: `User.Name == "x"`.
2. **Лямбда** — `p => p.Title == "111"`: имя параметра произвольное.
3. **Старая форма** — `post.Title == "111"`: префикс `post` по-прежнему поддерживается в любых выражениях.

Если выражение начинается не с поля сущности (например, с переменной контекста), оно передаётся интерпретатору как есть — в условиях доступны переменные страницы: `Where(CompanyId == companyId)`.

`OrderBy`/`ThenBy`/`Select`/`GroupBy` принимают имя поля или путь через точку: `OrderBy(User.Name)`, `Select(Title)`.

> `Last` требует предварительной сортировки (`OrderBy(...)`) — без неё EF не может определить «последний» элемент.
>
> `Select` превращает текущий запрос в проекцию: `ef.posts.Select(Title).ToList()` вернёт список заголовков, а последующие методы цепочки применяются уже к проекции.
>
> `ElementAt(i)` вернёт `null` при выходе индекса за границы; для определённого порядка требует предварительной сортировки `OrderBy`.
>
> `DistinctBy(f)` оставляет один элемент на каждое значение `f` (серверный `ROW_NUMBER() OVER (PARTITION BY f ...)`); какой именно дубликат выживет — определяет EF. `MaxBy(f)`/`MinBy(f)` возвращают элемент с максимальным/минимальным значением поля (транслируются в `ORDER BY f LIMIT 1`).
>
> `GroupBy(f)` — терминальный: возвращает список групп `EfGrouping { Key, Items, Count }` (не «сырой» `IGrouping`), запрос не изменяет. В шаблонах: `{{#each groups}}{{Key}} ({{Count}}): {{#each Items}}...{{/each}}{{/each}}`.

## Supported Query ef.<type>.<Method> methods
| Helper Name         | Example                          | Description |
|---------------------|----------------------------------|-------------|
| All                 | `.All(@expr)`                   | Проверяет, что все элементы удовлетворяют условию. Формы `@expr` как у `Where`. |
| Any                 | `.Any(@expr?)`                  | Проверяет наличие элементов. `@expr` необязательно — условие фильтрации (формы как у `Where`). |
| Average             | `.Average(@fieldName)`          | Среднее значение числового поля; запрос не изменяет. `@fieldName` — имя поля или путь через точку. |
| Count               | `.Count(@expr?)`                | Возвращает количество элементов в запросе. `@expr` необязательно — условие фильтрации (формы как у `Where`). |
| Distinct            | `.Distinct()`                   | Убирает дубликаты элементов из запроса. |
| DistinctBy          | `.DistinctBy(@fieldName)`       | Оставляет один элемент на каждое значение поля. `@fieldName` — имя поля или путь через точку. Какой именно дубликат выживет — определяет EF (серверный `ROW_NUMBER`). |
| ElementAt           | `.ElementAt(@index)`            | Возвращает элемент по индексу (0-based); запрос не изменяет. Вернёт `null`, если индекс вне диапазона. Требует `OrderBy` для определённого порядка. |
| First               | `.First(@expr?)`                | Возвращает первый элемент; запрос не изменяет. `@expr` необязательно — условие фильтрации. |
| GroupBy             | `.GroupBy(@fieldName)`          | Группирует элементы по полю и возвращает список групп `{Key, Items, Count}`; запрос не изменяет. `@fieldName` — имя поля или путь через точку. |
...
| Last                | `.Last(@expr?)`                 | Возвращает последний элемент; запрос не изменяет. Требует предварительной сортировки `OrderBy`. |
| Max                 | `.Max(@fieldName)`              | Возвращает максимальное значение поля; запрос не изменяет. `@fieldName` — имя поля или путь через точку. |
| MaxBy               | `.MaxBy(@fieldName)`            | Возвращает элемент с максимальным значением поля; запрос не изменяет. `@fieldName` — имя поля или путь через точку. |
| Min                 | `.Min(@fieldName)`              | Возвращает минимальное значение поля; запрос не изменяет. `@fieldName` — имя поля или путь через точку. |
| MinBy               | `.MinBy(@fieldName)`            | Возвращает элемент с минимальным значением поля; запрос не изменяет. `@fieldName` — имя поля или путь через точку. |
| OrderBy             | `.OrderBy(@fieldName)`          | Сортирует элементы по указанному полю. `@fieldName` — имя поля или путь через точку (`User.Name`). |
| OrderByDescending   | `.OrderByDescending(@fieldName)`| Сортирует элементы по указанному полю в порядке убывания. `@fieldName` — имя поля или путь через точку. |
| Search              | `.Search(@searchText)`          | Поиск по тексту. `@searchText` — текст для поиска. |
| Select              | `.Select(@expr)`                | Проецирует элементы в поле. `@expr` — имя поля или путь через точку; последующие методы применяются к проекции. |
| Skip                | `.Skip(@expr)`                  | Пропускает указанное количество элементов. `@expr` — выражение для вычисления количества элементов для пропуска. |
| Sum                 | `.Sum(@fieldName)`              | Сумма значений числового поля; запрос не изменяет. `@fieldName` — имя поля или путь через точку. |
