...
>
> `DistinctBy`, `MaxBy`, `MinBy`, `ElementAt` намеренно не добавлены — EF Core не транслирует их в SQL. Эмуляция: `MaxBy(f)` ≈ `OrderBy(f).Last()`, `ElementAt(i)` ≈ `Skip(i).Take(1).First()`.
