# AppFront
Марс может отобразить фронтэнд на многих технологиях

| Технология    | Пример    |
|---------------|-----------|
| Blazor        | готовится
| Razor         | готовится
| Handlebars    | https://github.com/mdimai666/MyMarsHandlebarsSiteTemplate

- можно установить дополнительные плагины шаблонизаторов.

Фронты — это папки с файлами (`data/fronts/<slug>` или внешняя папка), которые управляются из
админки в рантайме: создание, вкл/выкл, смена движка и URL без перезапуска.
Подробнее: [Фронты](md/AppFront/Fronts.md).

Или использовать как headless cms через REST api.

| lang      | link  |
|-----------|-------|
| c#        | https://www.nuget.org/packages/mdimai666.Mars.WebApiClient/
| js, ts    | готовится
| flutter   | готовится
