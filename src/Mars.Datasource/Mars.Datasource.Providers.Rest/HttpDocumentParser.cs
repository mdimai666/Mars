...
public static partial class HttpDocumentParser
{
    ...
    public static HttpDocument Parse(string? text)
    {
        ...
        for (var index = 0; index < lines.Length; index++)
        {
            ...
            {
                Add(document, block, index, lines);
        ...
        Add(document, block, lines.Length, lines);
    ...
    static void Add(HttpDocument document, Block? block, int end, string[] lines)
    {
        ...
        // Правило одно на фронт и сервер — см. DocumentText.EndLine.
        var blockText = string.Join('\n', lines.Skip(block.Line - 1).Take(Math.Max(end - block.Line + 1, 0)));
        ...
        document.Requests.Add(new HttpDocumentRequest
        {
            ...
            Line = block.Line,
            EndLine = DocumentText.EndLine(block.Line, blockText),
            Raw = raw,
        });
    }
