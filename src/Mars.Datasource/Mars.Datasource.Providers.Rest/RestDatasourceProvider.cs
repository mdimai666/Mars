...
public class RestDatasourceProvider : IDatasourceDiscoverableProvider
{
    ...
    public RestDatasourceProvider(DatasourceConfig config, IDatasourceStore store, RestHttpClientCache clients,
        IEnumerable<IRestCatalogDiscovery> discoveries, DatasourceKindProfile? profile = null)
    {
        _config = config;
        ...
        Profile = profile ?? RestDatasourceProfile.Create();
    }
    ...
    public async Task<DatasourceModifyResult> Modify(DatasourceRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            var document = ParseDocument(request.Query);
