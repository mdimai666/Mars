...
public static class RestResponseMapping
{
    ...
    static void FillArray(QueryResultDto result, JsonArray array, int maxRows)
    {
        ...
        if (items.Count > 0 && items.Count == array.Count)
        {
            ...
            result.Fields = Fields(shown);
            result.Rows = shown
                .Select(item => result.Fields.Select(column => Text(item[column.Name])).ToArray())
                .ToArray();
        ...
        result.Fields = [Field("value", array.Take(limit).ToList())];
    ...
    static void FillDocument(QueryResultDto result, JsonNode? body, string raw)
    {
        ...
        {
            result.Fields = Fields([item]);
            result.Rows = [result.Fields.Select(column => Text(item[column.Name])).ToArray()];
        ...
        if (body is not null)
        {
            result.Fields = [Field("value", [body])];
        ...
        result.Fields = [new DatasourceField { Name = "response", DataTypeName = JsonTypeInference.Text, ClrTypeName = typeof(string).FullName!, IsNullable = true }];
        result.Rows = string.IsNullOrWhiteSpace(text) ? [] : [[text]];
        result.Truncated = raw.Length > MaxJsonChars;
    }

    static DatasourceField[] Fields(IReadOnlyList<JsonObject> items)
    {
        List<string> names = [];
        ...
        return names
            .Select(name => Field(name, items.Take(SampleValues).Select(item => item[name]).ToList()))
            .ToArray();
    }

    static DatasourceField Field(string name, IReadOnlyList<JsonNode?> values)
    {
