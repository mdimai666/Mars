...
{
    public UpdatePostJsonQueryValidator(IMetaModelTypesLocator metaModelTypesLocator,
                                        ...
                                        IMetaValuesValidator metaValuesValidator)
    {
        RuleFor(x => x)
            .CustomAsync(async (x, context, cancellationToken) =>
            {
                ...
                foreach (var error in await metaValuesValidator.ValidateJsonAsync(postType.MetaFields, x.Meta, requireAll: false, ownerContext, cancellationToken))
