...
[KeyredHandler(key: FormEditorCatalog.BlockEditor, Tags = ["post"])]
internal class BlockEditor1PostContentProcessor(IPostRepository postRepository, IMetaModelTypesLocator metaModelTypesLocator) : IPostContentProcessor
{
    ...
    public Task<string?> RenderPostContent(PostTypeDetail postType, string content, CancellationToken cancellationToken)
    {
        if (postType.ContentEditorKey() != FormEditorCatalog.BlockEditor)
            throw new NotSupportedException($"{nameof(BlockEditor1PostContentProcessor)} is support only '{FormEditorCatalog.BlockEditor}'. Retrieved PostType '{postType.TypeName}'");
