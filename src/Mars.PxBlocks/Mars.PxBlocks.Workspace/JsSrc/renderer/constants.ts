// Ported from microsoft/pxt (MIT): pxtblocks/plugins/renderer/constants.ts
import * as Blockly from "blockly";
import { shapeForType } from "../types";
...
export class ConstantProvider extends Blockly.zelos.ConstantProvider {
    ...
    */
    ...
    override shapeFor(connection: Blockly.RenderedConnection) {
        ...
        // Формы из реестра типов (PxTypeRegistry из C#) имеют приоритет.
        if (checks) {
            for (const check of checks) {
                const shape = shapeForType(check);
                if (shape === 'hexagonal') {
        ...
        switch (connection.type) {
            ...
            case Blockly.ConnectionType.OUTPUT_VALUE:
                // Respect the type checks instead of inheriting the shape from the parent.
