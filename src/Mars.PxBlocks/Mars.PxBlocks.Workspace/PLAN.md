...
состояния при неизвестном контексте — Started=false + Dispose); `PxValueJsonTests`
(JSON → PxValue: примитивы, объект/массив рекурсивно, числа, созданные в памяти),
`PxInitialVariablesTests` (рантайм — перезапись по имени, нулевой дефолт, игнор
неизвестных имён и id; хост — значения запроса доходят до программы, ошибка
конвертации → Started=false) и `PxRunStateTests` (инъекция состояния конструктором,
предпочтение state-конструктора, экземпляр на запуск с кэшем на запуск,
`GetState<T>` без состояния и с чужим типом — ошибка). Тесты нашли дефект:
`PxValueJson` не принимал числа, созданные в памяти (int/long/decimal — не
JsonElement) — исправлено фолбэком по JSON-тексту. Всего 130 тестов.
