...
public class WordPressDatasourceTests : IClassFixture<WordPressFixture>
{
    ...
    /// <summary>Доступ источника — общий <see cref="AuthConfig"/> одним JSON-значением настройки.</summary>
    RestDatasourceProvider Provider(bool basicAuth = false) => new(
        new DatasourceConfig
        {
            ...
            Settings = new Dictionary<string, string>
            {
                ...
                [DatasourceSettings.Auth] = basicAuth
                    ? JsonSerializer.Serialize(new AuthConfig { Mode = AuthMode.BasicAuth, Username = "admin", Password = "admin" })
                    : "",
            },
        },
        new MemoryStore(),
    ...
    public async Task Rest_DiscoveryBuildsCatalogFromWordPressIndex()
    {
        ...
        catalog.Profile.Kind.Should().Be(DatasourceKind.Rest);
        ...
        posts.Operation!.Parameters.Select(parameter => parameter.Name).Should().Contain("per_page");

        var post = operations.Single(operation => operation.Id == "GET /wp-json/wp/v2/posts/{id}");
        post.Operation!.Parameters.Should().Contain(parameter => parameter.Name == "id" && parameter.In == DatasourceParameterIn.Path);
    }
    ...
    public async Task Rest_QueryReadsPostsAsTable()
    {
        ...
        result.Ok.Should().BeTrue(result.Message);
        result.Fields.Select(column => column.Name).Should().Contain(["id", "slug", "status"]);
    ...
    {
        var result = await Provider(basicAuth: true).Query(Request("GET {{baseUrl}}/wp-json/wp/v2/users/me?context=edit"));
    ...
    {
        var provider = Provider(basicAuth: true);
        ...
        var id = created.Fields.Select((column, index) => (column.Name, index))
