...
public class CatalogMappingTests
{
    ...
    {
        var catalog = CatalogMapping.FromStructure(Structure(), new DatasourceConfig(), SqlProfile());

        catalog.SourceName.Should().Be("mars");
        catalog.Profile.Kind.Should().Be(DatasourceKind.Sql);
    ...
    public void FromStructure_ColumnsOrderedByOrdinalWithCatalogFlags()
    {
        ...
        todo.Fields.Select(c => c.Name).Should().Equal("id", "title");

        var id = todo.Fields[0];
        ...
        var json = catalog.Groups[1].Objects[0].Fields.Single();
    ...
    [Fact]
    public void FromStructure_KeepsProfileOfSource()
    {
        var profile = SqlProfile();

        var catalog = CatalogMapping.FromStructure(Structure(), new DatasourceConfig(), profile);

        catalog.Profile.Should().BeSameAs(profile);
        catalog.Profile.Has(DatasourceFeature.Views).Should().BeTrue();
    }
    ...
    /// <summary>Профиль собирает провайдер источника; маппинг обязан донести его до каталога как есть.</summary>
    static DatasourceKindProfile SqlProfile() => new()
    {
        Kind = DatasourceKind.Sql,
        Features = [DatasourceFeature.Query, DatasourceFeature.Browse, DatasourceFeature.Write, DatasourceFeature.Views],
    };
}
