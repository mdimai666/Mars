...
public class RestDatasourceProviderTests
{
    ...
    public async Task Catalog_DocumentRequestCarriesBlockLines()
    {
        ...
        operation.Operation!.Line.Should().Be(1);
        operation.Operation!.EndLine.Should().Be(3);
    }
    ...
    public async Task Catalog_DiscoveryOperationHasNoBlockLines()
    {
        ...
        operation.Operation!.Line.Should().Be(0);
        operation.Operation!.EndLine.Should().Be(0);
    }
    ...
    [Fact]
    public void Profile_AllowsQueryBrowseWriteAndDocument()
    {
        var profile = new RestSource().Provider.Profile;
    ...
    public void AddDatasourceRest_RegistersFactoryAndDiscoveries()
    {
        ...
        var factory = provider.GetServices<IDatasourceProviderFactory>().Single(item => item.Profile.Kind == DatasourceKind.Rest);

        factory.Profile.Driver.Should().Be("");
        factory.Profile.KindLabel.Should().Contain("REST");
        factory.Profile.Label.Should().Contain("WordPress");
        factory.Profile.DefaultConnectionString.Should().Be("");
